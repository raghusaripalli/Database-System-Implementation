...
const char *dbfile_dir = "binFiles/"; // dir where binary heap files should be stored
const char *tpch_dir = "tables/"; // dir where dbgen tpch files (extension *.tbl) can be found
...
void test1 () {
	...
	char *rp = strdup(rel->path());
	dbfile.Create (rp, heap, NULL);
