...
Qin(in), Qout(out), Qsortorder(sortorder), Qrunlen(runlen), mysorter(sortorder), mysorter2(sortorder) {
	if (runlen > 0) {
		...
		curPage = new Page();
		theFile = new File();
		string name = tmpfName();
		char *randomFileName = strdup(name.c_str());
		theFile->Open(0, randomFileName);  // need a random name otherwise two or more bigq's would crash
	}
}
